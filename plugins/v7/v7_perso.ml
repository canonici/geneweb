...
and eval_compound_var conf base env (a, _ as ep) loc =
  ...
  | "sosa_anc" :: s :: sl ->
    ...
    *)
    begin match get_env "sosa_ref" env with
      | Vsosa_ref v -> begin match Lazy.force v with
          | (Some p) ->
            let ip = get_iper p in
...
and eval_person_field_var conf base env (p, p_auth as ep) loc =
  ...
  | [s] ->
      begin try bool_val (eval_bool_person_field conf base env ep s) with
        Not_found ->
          begin try eval_str_person_field conf base env ep s with
...
and eval_bool_person_field conf base env (p, p_auth) =
  ...
  | "has_children" ->
      begin match get_env "fam" env with
        Vfam (_, fam, _, _) ->
          Array.length (get_children fam) > 0
      | _ ->
          let b =
            Array.exists
              (fun ifam ->
...
and string_of_int_env var env =
  ...
  | _ -> raise Not_found
and obsolete_eval conf base env (p, _) loc = function
  | "married_to" ->
    let s =
      match get_env "fam" env with
        Vfam (_, fam, _, m_auth) ->
        ...
        Printf.sprintf (fcapitale format)
          (fun _ -> if m_auth then (string_of_marriage_text conf base fam : Adef.safe_string :> string) else "")
      | _ -> raise Not_found
    in
