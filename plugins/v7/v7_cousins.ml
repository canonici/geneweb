    ...
    | u ->
      Array.iteri
        (fun i ifam ->
           ...
           let sp =
             if get_sex p2 = Female
             then pget conf base (get_father cpl)
...
let rec print_descend_upto conf base cnt cnt_sp max_cnt ini_p ini_br lev children print print_sosa =
  if lev > 0 && cnt < max_cnt then begin
    if print then Output.print_sstring conf "<ul>";
    let (cnt, cnt_sp) =
      List.fold_left
 	(fun (cnt, cnt_sp) (ip, ia_asex, rev_br) ->
 	   ...
 	   then
 		 ...
 		 begin
 		   ...
 		   if lev = 1 then give_access conf base ia_asex ini_p ini_br p br print_sosa
 		   else
 		 ...
 		 List.fold_left
 	       ...
 	       in
 	       if lev <= 2 && print then Output.print_sstring conf "</li>";
 	       if lev = 1 then
...
let print_cousins_side_of conf base cnt cnt_sp max_cnt a ini_p ini_br lev1 lev2 print print_sosa =
  ...
  if List.exists (sibling_has_desc_lev conf base lev2) sib then begin
    ...
    let (cnt, cnt_sp) =
      print_descend_upto conf base cnt cnt_sp max_cnt ini_p ini_br lev2 sib print print_sosa
    in
    if lev1 > 1 then Output.print_sstring conf "</li>";
    (true, cnt, cnt_sp)
  end else (false, cnt, cnt_sp)
        ...
        let (some, cnt, cnt_sp) =
          match Util.old_branch_of_sosa conf base (get_iper p) sosa with
            ...
            let (some1, cnt, cnt_sp) =
              print_cousins_side_of
                conf base cnt cnt_sp max_cnt (pget conf base ia) p br
		lev1 lev2 print print_sosa
            in (some1 || some, cnt, cnt_sp)
          | _ -> (some, cnt, cnt_sp)
        in
...
let print_cousins conf base p lev1 lev2 =
  ...
  if cnt >= max_cnt then Output.print_sstring conf "etc... "
    ...
    Output.print_sstring conf " (" ;
    Output.print_sstring conf (string_of_int cnt2) ;
    Output.print_sstring conf ") " ;
  ...
  if p_getenv conf.env "spouse" = Some "on" then begin
    ...
    Output.print_sstring conf " (" ;
    Output.print_sstring conf (string_of_int cnt2_sp) ;
    Output.print_sstring conf ") " ;
